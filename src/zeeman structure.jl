...
function zeeman_struc(atom::Atom, level_num::Int = 0, BF::BField = 0u"Gauss")
    BFu = ustrip(Float64, u"Gauss", BF)
    level = level_num == 0 ? atom.ground : atom.excited[level_num]
    (; L, S, J) = level.state
    I = atom.I
    (; A, B) = level.hfc
    Au::Float64 = ustrip(Float64, u"cm^(-1)", A)
    Bu::Float64 = ustrip(Float64, u"cm^(-1)", B)
    ...
    for MF in basis_df.MF
        ...
        gl = 1 - 𝑚e / atom.M
        h = hamiltonian_total(basis1, Au, Bu, BFu, μB = 𝜇B, μN = 𝜇N, gl = gl, gs = gS, gI = atom.gI)
